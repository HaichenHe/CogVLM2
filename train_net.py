    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(
        ...
        if cfg.TRAIN.LINEAR_CONNECT_CLIMB:
            ...
            with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
                ...
                preds1, preds2 = model(inputs, labels, index)
                
                # Compute the loss.
                loss1 = loss_fun(preds1, labels)
                loss2 = loss_fun(preds2, labels)
                loss = loss1 + 4.0 * loss2
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel":
                ...
                preds, labels = model(inputs, labels)
            else:
                if cfg.MODEL.RECORD_ROUTING:
                    preds, rout_state = model(inputs, labels)
                    ...
                    preds, raw_preds = model(inputs, labels)
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                # Compute the errors.
                num_topks_correct = metrics.topks_correct(preds1, labels, (1, 5))
                top1_err, top5_err = [
                    (1.0 - x / preds1.size(0)) * 100.0 for x in num_topks_correct
                ]
...
def train(cfg):
    ...
    if writer is not None:
        writer.close()
    result_string = (
        "_t{:.2f}_m{:.2f} _a{:.2f} Top5 Acc: {:.2f} MEM: {:.2f}"
        "".format(
